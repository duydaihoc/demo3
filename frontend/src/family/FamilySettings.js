...
export default function FamilySettings() {
	...
	// fetch family data helper (re-usable)
	const fetchFamilyData = useCallback(async () => {
		...
		try {
			...
			if (!familyRes.ok) throw new Error('Không thể tải thông tin gia đình');
	...
	}, [token, selectedFamilyId, API_BASE, navigate, getCurrentUser]);
	
	// Lấy thông tin gia đình từ API
	useEffect(() => { fetchFamilyData(); }, [fetchFamilyData]);
